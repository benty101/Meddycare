...
export function EmbeddedCarerQuestionnaire() {
  ...
  const progress = ((currentStep + 1) / carerQuestions.length) * 100;
    ...
    if (currentQuestion.type === 'single-choice') {
      setTimeout(() => {
        if (currentStep < carerQuestions.length - 1) {
          ...
          router.push('/carers/questionnaire');
        }
      }, 300);
    }
  };
    ...
    <section className="py-20 bg-(--brand-purple-50)">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-urbanist font-semibold text-gray-600">
              Question {currentStep + 1} of {carerQuestions.length}
            </span>
            <span className="text-sm font-urbanist font-semibold text-gray-600">
        ...
        <p className="text-center text-sm text-gray-500 font-inter">
          Your information is secure and will only be used to match you with suitable families
        </p>
      </div>
    </section>
  );
